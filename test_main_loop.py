...
def main(session: SetsSession):
    ...
    item_manager = ItemManager(session=session)
    blocker = Blocker()
    buyer = BuyMarket()
    seller = SellMarket()
    print("Данные созданы и заполнены")
    ...
    while True:
        for app in MONITOR_APPS:
            ...
            if new_items:
                ...
                for key, val in new_items.items():
                    ...
                    print(f"заказ {order} -> предмет {item}")
            ...
            _item_name = "Winter 2024 Cosmetic Case"  # inter 2024 Cosmetic Case || Scream Fortress XVI War Paint Case
        ...
        if new_buy_items:
            print(f"\nзаказ {list(new_buy_items.keys())} найден")
            for key, val in new_buy_items.items():
                ...
                print(f"добавлено {val['quantity']} новых заказов для игры {val['appid']}")
        ...
        if selling_items or len(item_manager.item_hub) > 0:
            ...
            for it in deleted_items:
                print(f'удален из списка предметов: {it} -> {round(it.sell_price / 100, 2)} руб')
